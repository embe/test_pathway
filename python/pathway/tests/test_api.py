...
from .utils import assert_equal_tables, assert_equal_tables_wo_index, xfail_on_rust
...
def static_table_from_md(scope, txt, ptr_columns=()) -> api.Table:
    ...
    return scope.static_table_from_pandas(df, {}, unsafe_trusted_ids=True)
...
@xfail_on_rust
def test_n_join_on_id(event_loop):
    def build(s):
        latin = static_table_from_md(
            s,
            """
            lower upper
        1       a     A
        ...
        njoiner = s.join_on_id(
            [latin.universe, arabic.universe, persian.universe], api.JoinType.INNER
        )
        ret = s.table(
            njoiner.universe,
            [
                njoiner.select_column(latin.columns[1]),
                njoiner.select_column(persian.columns[0]),
                njoiner.select_id_column(),
            ],
        )
        expected = static_table_from_md(
            s,
...
def test_flat_map(event_loop):
    def build(s):
        ...
        computed_col = s.flat_map(
            tab, lambda values: [values[1] for _ in range(values[0])], int
        )
        ret = s.table(computed_col.universe, [computed_col])
        expected = static_table_from_md(
            s,
            """
        c0
...
def test_iteration(event_loop):
    ...
    def build(s):
        iterated = [
            s.static_table_from_pandas(
                pd.DataFrame(index=range(1, 101), data={"c0": np.arange(1.0, 101.0)})
            )
        ]

        ([result_tab], []) = s.iterate(iterated, [], [], collatz_logic)

        expected = s.static_table_from_pandas(
            pd.DataFrame(index=range(1, 101), data={"c0": 1.0})
        )
...
def test_iteration_limit(limit, event_loop):
    ...
    def build(s):
        ...
        iterated = [s.static_table_from_pandas(pd.DataFrame(data={"c0": values}))]

        ([result_tab], []) = s.iterate(iterated, [], [], logic, limit=limit)

        expected = s.static_table_from_pandas(
            pd.DataFrame(data={"c0": [v + limit for v in values]})
        )
