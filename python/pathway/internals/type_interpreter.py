...
from pathway.internals.dtype import (  # type: ignore
    DType,
    dtype_issubclass,
    types_lca,
    unoptionalize,
...
from pathway.internals.types import DateTimeNaive, DateTimeUtc, Duration
...
class TypeInterpreter(ExpressionVisitor):
    ...
    def eval_ix(self, expression: expr.ColumnIxExpression) -> DType:
        if expression._optional:
            return Optional[expression._column.dtype]
        else:
            return expression._column.dtype
    ...
    def eval_require(self, expression: expr.RequireExpression) -> DType:
        return Optional[self.eval_expression(expression._val)]
