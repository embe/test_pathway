...
class ColumnExpression(OperatorInput, ABC):
    ...
    def __add__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.add)

    def __radd__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.add)

    def __sub__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.sub)

    def __rsub__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.sub)

    def __mul__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.mul)

    def __rmul__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.mul)

    def __truediv__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.truediv)

    def __rtruediv__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.truediv)

    def __floordiv__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.floordiv)

    def __rfloordiv__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.floordiv)

    def __mod__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.mod)

    def __rmod__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.mod)

    def __pow__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.pow)

    def __rpow__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.pow)

    def __lshift__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.lshift)

    def __rlshift__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.lshift)

    def __rshift__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.rshift)

    def __rrshift__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
    ...
    def __and__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.and_)

    def __rand__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.and_)

    def __or__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.or_)

    def __ror__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(other, self, operator.or_)

    def __xor__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
        return ColumnBinaryOpExpression(self, other, operator.xor)

    def __rxor__(self, other: ColumnExpressionOrConst) -> ColumnBinaryOpExpression:
    ...
    def get(
        ...
        default: ColumnExpressionOrConst = None,
    ) -> ColumnExpression:
        """Extracts element at `index` from an object. The object has to be a Tuple.
...
ColumnExpressionOrConst = Union[ColumnExpression, Value]
...
class ColumnCallExpression(ColumnExpression):
    ...
    def __init__(
        self, col_expr: ColumnRefOrIxExpression, args: Iterable[ColumnExpressionOrConst]
    ):
        super().__init__()
...
class ColumnBinaryOpExpression(ColumnExpression):
    ...
    def __init__(
        self,
        left: ColumnExpressionOrConst,
        right: ColumnExpressionOrConst,
...
class ColumnUnaryOpExpression(ColumnExpression):
    ...
    def __init__(self, expr: ColumnExpressionOrConst, operator: Callable[[Any], Any]):
        super().__init__()
...
class ReducerExpression(ColumnExpression):
    ...
    def __init__(self, reducer: Callable, *args: ColumnExpressionOrConst):
        super().__init__()
...
class ApplyExpression(ColumnExpression):
    ...
    def __init__(
        ...
        fun: Callable,
        return_type=None,
        *args: ColumnExpressionOrConst,
        **kwargs: ColumnExpressionOrConst,
    ):
        super().__init__()
        self._fun = fun
...
class CastExpression(ColumnExpression):
    ...
    def __init__(self, return_type: Any, expr: ColumnExpressionOrConst):
        super().__init__()
        self._return_type = return_type
        self._expr = _wrap_arg(expr)
        self._deps = [self._expr]
...
class CoalesceExpression(ColumnExpression):
    ...
    def __init__(self, *args: ColumnExpressionOrConst):
        super().__init__()
        assert len(args) > 0
        self._args = tuple(_wrap_arg(arg) for arg in args)
        self._deps = self._args
...
class RequireExpression(ColumnExpression):
    ...
    def __init__(self, val: ColumnExpressionOrConst, *args: ColumnExpressionOrConst):
        super().__init__()
...
class IfElseExpression(ColumnExpression):
    ...
    def __init__(
        self,
        _if: ColumnExpressionOrConst,
        _then: ColumnExpressionOrConst,
        _else: ColumnExpressionOrConst,
    ):
        super().__init__()
...
class PointerExpression(ColumnExpression):
    ...
    def __init__(
        self, table: Table, *args: ColumnExpressionOrConst, optional=False
    ) -> None:
        super().__init__()
...
def _wrap_arg(arg: ColumnExpressionOrConst) -> ColumnExpression:
