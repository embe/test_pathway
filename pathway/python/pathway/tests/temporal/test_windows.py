...
from pathway.internals.types import DateTimeNaive
...
def test_windows_with_datetimes(w):
    ...
    if w == pw.temporal.session(max_gap=datetime.timedelta(minutes=10)):
       ...
       4 |   6   |   6
        ...
        if api._API_VARIANT == "python":
            pytest.xfail(
                reason="Equal entries in the case of python also don't work"
                + ", but different rows get merged than in rust"
            )
