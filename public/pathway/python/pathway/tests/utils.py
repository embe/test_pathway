...
xfail_on_rust = pytest.mark.xfail(
    api._API_VARIANT == "rust", reason="not (yet) supported on Rust variant"
)
xfail_on_python = pytest.mark.xfail(
    api._API_VARIANT == "python", reason="not (yet) supported on Python variant"
)
